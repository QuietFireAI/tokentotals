  ...
  <header class="tt-chat-topbar">
    <div class="tt-chat-brand">
      ...
      <div class="tt-chat-controls">
        ...
        <button id="exportThreadButton" type="button" disabled>Export CSV</button>
...
const exportThreadButton = document.getElementById("exportThreadButton");
let messages = [];
let sending = false;
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      if (response.ok) {
        ...
        exportThreadButton.disabled = false;
        return;
      }
...
function exportThreadCsv() {
  if (exportThreadButton.disabled) return;
  const link = document.createElement("a");
  link.href = `/api/threads/${encodeURIComponent(threadId)}/turn-receipts.csv`;
  link.download = "TokenTotals_Thread_Receipts.csv";
...
function resetThread() {
  ...
  exportThreadButton.disabled = true;
...
exportThreadButton.addEventListener("click", exportThreadCsv);
