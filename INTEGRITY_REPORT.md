...
**Validation:** regression tests prove candidate parsing, failed-check preservation, runtime consumption of the promoted snapshot, suspicious-jump quarantine, same-day retry after failure, and review-required handling for source-only changes. The post-fix GitHub Actions suite passed 19 tests with 0 failures on Ubuntu/Python 3.12.
...
**Repair:** wording changed to dated provider receipts. OpenAI now has a separately validated official-source checker; Anthropic and Google remain explicitly described as dated verified catalog entries until their own adapters are implemented.
...
**Repair:** matrix is generated by `generate_model_matrix.py` from the effective verified pricing view supplied by `pricing_engine`.
...
Current GitHub Actions regression suite on the hardened branch: **19 passed / 0 failed** on Ubuntu/Python 3.12 after the IR-003 implementation and test-isolation correction.
...
- The 19-test suite is focused regression coverage, not a full integration or load test.
